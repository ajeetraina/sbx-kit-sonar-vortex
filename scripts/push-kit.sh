#!/usr/bin/env bash
#
# Publish the sonar-vortex kit to Docker Hub as an OCI artifact.
#
# Usage:
...
#   REPO             repository name             (default: sonar-vortex-kit)
#   TAG              image tag                   (default: latest, or $1)
#   DOCKERHUB_TOKEN  Docker Hub access token / PAT (never echoed)
#   SIGN             set to 1 to Sigstore-sign the push (keyless)
...
echo "Consume with:  sbx run claude --kit $REF --kit-arg org=<your-org> ."
